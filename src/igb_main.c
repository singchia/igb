...
static void igb_xdp_ring_update_tail(struct igb_ring *ring)
{
	 ...
	 */
	wmb();
	write(ring->next_to_use, ring->tail);
}
...
static int igb_xmit_back(struct igb_adapter *adapter, struct xdp_buff *xdp)
{
	struct xdp_frame *xdpf = xdp_convert_buff_to_frame(xdp);
	int cpu = smp_processor_id();
	struct igb_ring *tx_ring;
	struct netdev_qaueue *nq;
	u32 ret;
	 ...
	 * but ring has not been configured yet. In this case simple abort xmit.
	 */
